...
def Conv1DTranspose(input_tensor, filters, kernel_size, padding='same'):
    ...
    x = Conv2DTranspose(filters=filters, kernel_size=(
        kernel_size, 1), padding=padding)(x)
...
def generate_traces(model, labeled_traces_file, trace_data, trace_data_o):
    ...
    all_traces = unscale(noiseFilter(trace_data, 0, model)
                         [:, :, 0], trace_data_o)
    ...
    out_file.create_dataset('Profiling_traces/traces',
                            data=np.array(profiling_traces, dtype=np.int16))

    print("Processing attack traces...")
    attack_traces = all_traces[50000:]
    print("Attack: after")
    del out_file['Attack_traces/traces']
    out_file.create_dataset('Attack_traces/traces',
                            data=np.array(attack_traces, dtype=np.int16))
    ...
    out_file.close()
    print("Denoiser training finalized!")


if __name__ == "__main__":
    Noisy_traces = "Noisy.h5"
    Target_trace = "Clean.h5"
    Denoised_traces = "Denoised.h5"

    Noisy_root = "/Trace/Noisy_traces/"
    Clean_root = "/Trace/Clean_traces/"
    Denoised_root = "/Trace/Denoised_traces/"

    model_dir = "/Model/Denoise_model_" + Denoised_traces
    save_model = ModelCheckpoint(model_dir)
    callbacks = [save_model]

    # load traces
    print('load traces...')
    X_noisy_o = load_ascad(Noisy_root + Noisy_traces)
    Y_clean_o = load_ascad(Clean_root + Target_trace)
    ...
    autoencoder.fit(X_noisy[:10000], Y_clean[:10000],
                    validation_data=(X_noisy[45000:50000], Y_clean[45000:50000]),
                    epochs=100,
                    batch_size=128,
                    verbose=2,
                    callbacks=callbacks)

    generate_traces(autoencoder, Denoised_root + Denoised_traces, X_noisy, Y_clean_o)
    SNR(Denoised_root + Denoised_traces)
